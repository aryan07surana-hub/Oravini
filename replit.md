...
- Auto-sync cron: node-cron job in server/cron.ts, runs daily at 3AM UTC, syncs Instagram post stats — tries Meta Graph API first (for linked account), falls back to Apify; also accessible via POST /api/admin/auto-sync
