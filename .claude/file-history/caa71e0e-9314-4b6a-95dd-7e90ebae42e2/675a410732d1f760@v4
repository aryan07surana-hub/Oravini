...
import { insertUserSchema, insertDocumentSchema, insertProgressSchema, insertCallFeedbackSchema, insertTaskSchema, insertNotificationSchema, insertContentPostSchema, insertIncomeGoalSchema, insertUserFeedbackSchema, insertScheduledInstagramPostSchema, commentAutoReplies, storyReplyConfigs, dmFlows, dmContactTags, aiBotConfigs, optInLinks, metaTokens } from "@shared/schema";
...
Return ONLY a JSON object (no markdown, no text outside JSON):
{
  ...
  app.get("/api/dm/comment-replies", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const qUserId = req.query.userId as string;
      const targetId = (req.user as any).role === "admin" && qUserId ? qUserId : userId;
      const rows = await db.select().from(commentAutoReplies).where(eq(commentAutoReplies.userId, targetId)).orderBy(desc(commentAutoReplies.createdAt));
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.post("/api/dm/comment-replies", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = req.body.userId || (req.user as any).id;
      const [row] = await db.insert(commentAutoReplies).values({ ...req.body, userId }).returning();
      res.json(row);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  // ── Story Reply Config ─────────────────────────────────────────────────────
  app.get("/api/dm/story-reply", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const [row] = await db.select().from(storyReplyConfigs).where(eq(storyReplyConfigs.userId, userId));
      res.json(row || null);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.put("/api/dm/story-reply", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const existing = await db.select().from(storyReplyConfigs).where(eq(storyReplyConfigs.userId, userId));
      let row;
      if (existing.length > 0) {
        [row] = await db.update(storyReplyConfigs).set({ replyMessage: req.body.replyMessage, isActive: req.body.isActive }).where(eq(storyReplyConfigs.userId, userId)).returning();
      } else {
        [row] = await db.insert(storyReplyConfigs).values({ userId, ...req.body }).returning();
      }
      res.json(row);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  // ── DM Visual Flows ────────────────────────────────────────────────────────
  app.get("/api/dm/flows", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const qUserId = req.query.userId as string;
      const targetId = (req.user as any).role === "admin" && qUserId ? qUserId : userId;
      const rows = await db.select().from(dmFlows).where(eq(dmFlows.userId, targetId)).orderBy(desc(dmFlows.createdAt));
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.post("/api/dm/flows", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = req.body.userId || (req.user as any).id;
      const [row] = await db.insert(dmFlows).values({ ...req.body, userId }).returning();
      res.json(row);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  // ── AI Bot Config ──────────────────────────────────────────────────────────
  app.get("/api/dm/ai-bot", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const [row] = await db.select().from(aiBotConfigs).where(eq(aiBotConfigs.userId, userId));
      res.json(row || { userId, isActive: false, personality: "friendly", instructions: "", fallbackMessage: "", keywordsOnly: false, keywords: [] });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.put("/api/dm/ai-bot", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const existing = await db.select().from(aiBotConfigs).where(eq(aiBotConfigs.userId, userId));
      let row;
      if (existing.length > 0) {
        [row] = await db.update(aiBotConfigs).set({ ...req.body, updatedAt: new Date() }).where(eq(aiBotConfigs.userId, userId)).returning();
      } else {
        [row] = await db.insert(aiBotConfigs).values({ userId, ...req.body }).returning();
      }
      res.json(row);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.post("/api/dm/ai-bot/test", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const { message } = req.body;
      const [config] = await db.select().from(aiBotConfigs).where(eq(aiBotConfigs.userId, userId));
      const apiKey = process.env.ANTHROPIC2_API_KEY || process.env.ANTHROPIC_API_KEY;
      if (!apiKey) return res.status(400).json({ message: "AI not configured" });
      const client = new Anthropic({ apiKey });
      const personality = config?.personality || "friendly";
      const instructions = config?.instructions || "";
      const systemPrompt = `You are an Instagram DM assistant. Be ${personality} and concise (max 3 sentences). ${instructions} Never reveal you are AI unless directly asked.`;
  ...
  // ── Opt-in Links ───────────────────────────────────────────────────────────
  app.get("/api/dm/opt-in-links", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const rows = await db.select().from(optInLinks).where(eq(optInLinks.userId, userId)).orderBy(desc(optInLinks.createdAt));
      res.json(rows);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.post("/api/dm/opt-in-links", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const refCode = Math.random().toString(36).substring(2, 8).toUpperCase();
      const [row] = await db.insert(optInLinks).values({ ...req.body, userId, refCode }).returning();
      res.json(row);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  // ── DM Conversations (Live Chat via Meta API) ──────────────────────────────
  app.get("/api/dm/conversations", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const token = await db.select().from(metaTokens).where(eq(metaTokens.userId, userId));
      if (!token[0]?.accessToken) return res.json({ conversations: [], connected: false });
      const igId = token[0].igAccountId;
      if (!igId) return res.json({ conversations: [], connected: false });
      const url = `https://graph.facebook.com/v18.0/${igId}/conversations?platform=instagram&fields=participants,messages{message,from,created_time}&access_token=${token[0].accessToken}&limit=20`;
      const response = await fetch(url);
      const data = await response.json() as any;
      if (data.error) return res.json({ conversations: [], error: data.error.message, connected: true });
      res.json({ conversations: data.data || [], connected: true });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.get("/api/dm/conversations/:igUserId/messages", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const token = await db.select().from(metaTokens).where(eq(metaTokens.userId, userId));
      if (!token[0]?.accessToken) return res.status(400).json({ message: "Instagram not connected" });
      const igId = token[0].igAccountId;
      const url = `https://graph.facebook.com/v18.0/${igId}/conversations?platform=instagram&user_id=${req.params.igUserId}&fields=messages{message,from,created_time,attachments}&access_token=${token[0].accessToken}`;
      const response = await fetch(url);
      const data = await response.json() as any;
