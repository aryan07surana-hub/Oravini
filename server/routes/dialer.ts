...
export function registerDialerRoutes(app: Express, requireAuth: any) {
  ...
  app.put("/api/dialer/settings", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const {
        ...
        aiProvider, vapiApiKey, vapiAssistantId, blandApiKey, blandVoiceId, aiSystemPrompt,
      } = req.body;
      ...
      const data = {
        ...
        vapiApiKey, vapiAssistantId, blandApiKey, blandVoiceId, aiSystemPrompt,
        updatedAt: new Date(),
      };
      if (existing) {
  ...
  app.post("/api/dialer/ai/call", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const [settings] = await db.select().from(dialerSettings).where(eq(dialerSettings.userId, userId));
      const provider  = settings?.aiProvider || "vapi";
  ...
  app.post("/api/dialer/ai/webhook/:provider", async (req: Request, res: Response) => {
    try {
      const provider = req.params.provider as "vapi" | "bland";
      const parsed = provider === "vapi" ? parseVapiWebhook(req.body) : parseBlandWebhook(req.body);
  ...
  app.post("/api/dialer/ai/campaigns/:id/start", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!apiKey) return res.status(503).json({ message: `${provider === "vapi" ? "Vapi" : "Bland AI"} API key not configured` });

      // Update campaign status
      await db.update(dialerAiCampaigns).set({
        status: "running", totalLeads: leads.length, startedAt: new Date(), updatedAt: new Date(),
      ...
      const firstBatch    = leads.slice(0, maxConcurrent);
      ...
      if (leads.length > maxConcurrent) {
        const remaining = leads.slice(maxConcurrent);
      ...
      res.json({ ok: true, totalLeads: leads.length, firstBatch: firstBatch.length });
    } catch (err: any) {
  ...
  app.post("/api/dialer/cadences/:id/enroll", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const [enrollment] = await db.insert(dialerCadenceEnrollments)
        .values({ id: randomUUID(), cadenceId: cadence.id, leadId, userId, nextRunAt })
        .returning();
      res.json(enrollment);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.get("/api/dialer/cadences/enrollments", requireAuth, async (req: Request, res: Response) => {
    try {
      const enrollments = await db.select().from(dialerCadenceEnrollments)
        .where(eq(dialerCadenceEnrollments.userId, uid(req)))
