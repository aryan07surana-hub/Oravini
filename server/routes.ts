...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // Serve HLS files generated by node-media-server
  const pathMod = await import("path");
  const fsMod = await import("fs");
  const hlsDir = pathMod.resolve("hls");
  if (!fsMod.existsSync(hlsDir)) fsMod.mkdirSync(hlsDir, { recursive: true });
  app.use("/hls", (await import("express")).default.static(hlsDir, {
    setHeaders: (res: any) => {
        ...
        switch (type) {
          ...
          case "relay_stop": {
            ...
            if (key) {
              ...
              ws.send(JSON.stringify({ type: "relay_stopped" }));
            }
            break;
          }
          case "wr_raise_hand": {
  ...
  // ── Anthropic helper (redirected to Groq) ────────────────────────────────
  async function callAnthropic(systemPrompt: string, userPrompt: string, maxTokens = 4000): Promise<string> {
    return callGroq(systemPrompt, userPrompt, Math.min(maxTokens, 8000));
  }

  // ── Groq cascade (deep analysis) ─────────────────────────────────────────
  async function callOpenRouter(systemPrompt: string, userPrompt: string, maxTokens = 4000): Promise<string> {
    try {
      ...
      console.warn("[Groq cascade] JSON mode failed, trying plain:", e.message?.slice(0, 120));
    }
    return callGroq(systemPrompt, userPrompt, Math.min(maxTokens, 8000));
  }
  ...
  app.post("/api/webinars/:id/start", requireAuth, async (req, res) => {
    try {
      ...
      if (webinar?.emailReminderEnabled) {
        ...
        console.log(`[webinar] Sending live notification to ${registrations.length} registrants for ${webinar.title}`);
      }
      res.json(updated);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
