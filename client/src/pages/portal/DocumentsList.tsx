...
export default function DocumentsList() {
            ...
            <button
              onClick={() => {
                ...
                if (currentFile.parentId) {
                  setSelectedSuper(currentFile.parentId);
                }
              }}
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder={docForm.type === "link" ? "Add any notes..." : "Paste or type your content..."}
                  rows={4}
                />
              </div>
    ...
    return (
      <PortalLayout>
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setSelectedSuper(null); setSearch(""); }}
              className="text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5 text-sm"
            >
              <ArrowLeft className="w-4 h-4" />
              Back
          ...
          {filteredChildren.length === 0 ? (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm">
                {search ? "No files match your search" : 'No files yet. Add one with "New File".'}
              </p>
            </div>
          ) : (
        ...
        <Dialog open={showNewFile} onOpenChange={setShowNewFile}>
          <DialogContent className="sm:max-w-sm">
            <DialogHeader>
              <DialogTitle>New File in "{currentSuper.name}"</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 pt-2">
              <div className="space-y-1.5">
  ...
  const filteredRoot = rootItems.filter(
    (f) => !search || f.name.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <PortalLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {rootItems.length} {rootItems.length === 1 ? "item" : "items"} &middot;{" "}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    >
                      {isSuper ? (
                        ...
                        <span className="text-lg font-bold">{f.name[0].toUpperCase()}</span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
      ...
      <Dialog open={showNewSuper} onOpenChange={setShowNewSuper}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>New Super File</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Oravini Docs, Client Files..."
                autoFocus
                onKeyDown={(e) => e.key === "Enter" && createSuperFile()}
              />
            </div>
                  ...
                  {movableFiles.map((f) => (
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{ background: `${fileColor(f.name)}22`, color: fileColor(f.name) }}
                      >
                        {f.name[0].toUpperCase()}
                      </div>
