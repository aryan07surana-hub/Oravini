...
import { FolderOpen, Plus, Trash2, Link2, FileText, ArrowLeft, Search, X, ExternalLink, Folder } from "lucide-react";
...
export default function DocumentsList() {
  ...
  const [docForm, setDocForm] = useState<{ name: string; type: DocType; url: string; content: string }>({ name: "", type: "link", url: "", content: "" });
  ...
  function createFile() {
    ...
    const f: DocFile = { id: uid(), name: newFileName.trim(), docs: [], createdAt: new Date().toISOString() };
  ...
  function addDoc() {
    ...
    const doc: Doc = { id: uid(), name: docForm.name.trim(), type: docForm.type, url: docForm.url.trim(), content: docForm.content.trim(), createdAt: new Date().toISOString() };
  ...
  const filteredDocs = currentFile
    ? currentFile.docs.filter((d) => !search || d.name.toLowerCase().includes(search.toLowerCase()) || d.url.toLowerCase().includes(search.toLowerCase()) || d.content.toLowerCase().includes(search.toLowerCase()))
    : [];
      ...
      <SuperAdminLayout>
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            <button onClick={() => { setSelectedFile(null); setSearch(""); }} className="text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5 text-sm">
            ...
            <div className="space-y-2">
              {filteredDocs.map((doc) => (
                <div key={doc.id} className="group rounded-xl border border-border bg-card p-4 flex items-start gap-3">
                  ...
                  <button onClick={() => setFiles((prev) => prev.map((f) => f.id === currentFile.id ? { ...f, docs: f.docs.filter((d) => d.id !== doc.id) } : f))} className="text-muted-foreground hover:text-destructive transition-colors opacity-0 group-hover:opacity-100 flex-shrink-0 mt-0.5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {files.length} {files.length === 1 ? "file" : "files"} &middot; {files.reduce((a, f) => a + f.docs.length, 0)} total docs
            </p>
          </div>
              ...
              return (
                <button key={f.id} onClick={() => { setSelectedFile(f.id); setSearch(""); }} className="group text-left rounded-xl border border-border bg-card p-4 hover:border-foreground/20 transition-all hover:shadow-md">
                  <div className="flex items-start gap-3">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setFiles((prev) => prev.filter((x) => x.id !== f.id)); }} className="text-muted-foreground hover:text-destructive transition-colors opacity-0 group-hover:opacity-100 flex-shrink-0">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </button>
