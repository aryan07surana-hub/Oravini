...
import {
  ...
  Star, FileText, Clock, Plus, Trash2, Pencil, BarChart2, ArrowLeft, Bell, ChevronRight
} from "lucide-react";
    ...
    <AdminLayout>
      <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">View and manage all client content performance</p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-4 flex-wrap">
              <div className="flex-1 min-w-[240px]">
                <Label className="text-xs text-muted-foreground uppercase tracking-wide mb-2 block">Select Client</Label>
                <Select value={selectedClientId} onValueChange={setSelectedClientId}>
                  <SelectTrigger data-testid="admin-client-select">
                    <SelectValue placeholder="Choose a client to view their tracking..." />
                  </SelectTrigger>
                  <SelectContent>
                    {clients.map((c: any) => (
              ...
              {selectedClient && (
                <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-lg px-4 py-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-primary">{selectedClient.name}</p>
        ...
        {!selectedClientId ? (
          <div className="text-center py-20 border border-dashed border-border rounded-2xl">
            ...
            <p className="text-sm font-medium text-foreground">Select a client above</p>
            <p className="text-xs text-muted-foreground mt-1">Choose a client to view their Instagram and YouTube tracking data</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-4">
