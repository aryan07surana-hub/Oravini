...
import {
  Clapperboard, Sparkles, Zap, DollarSign, Smile, Film, GraduationCap, User,
  Target, Copy, Check, ChevronDown, ChevronUp, Loader2, Link2, FileText, Lightbulb,
  Scissors, Volume2, Eye, ListChecks, Shuffle, Wand2, Music, Layers, Star,
  TrendingUp, Play, RotateCcw, Hash, Video, Camera, Download, CheckSquare, Square
} from "lucide-react";

const MODES = [
  { id: "viral", label: "Viral", emoji: "🔥", desc: "Fast cuts, bold captions, max energy", color: "from-red-500/20 to-orange-500/10 border-red-500/30" },
  { id: "story", label: "Story", emoji: "🎬", desc: "Narrative arc, emotional beats", color: "from-purple-500/20 to-purple-500/5 border-purple-500/30" },
  { id: "sales", label: "Sales", emoji: "💰", desc: "Problem → solution → CTA", color: "from-green-500/20 to-green-500/5 border-green-500/30" },
  { id: "funny", label: "Funny", emoji: "😂", desc: "Timing, punchlines, reaction cuts", color: "from-yellow-500/20 to-yellow-500/5 border-yellow-500/30" },
  { id: "cinematic", label: "Cinematic", emoji: "🎥", desc: "B-roll heavy, dramatic pacing", color: "from-blue-500/20 to-blue-500/5 border-blue-500/30" },
  { id: "educational", label: "Educational", emoji: "📚", desc: "Numbered points, clarity first", color: "from-cyan-500/20 to-cyan-500/5 border-cyan-500/30" },
  { id: "personal_brand", label: "Personal Brand", emoji: "🚀", desc: "Authentic voice, trust-building", color: "from-primary/20 to-primary/5 border-primary/30" },
];
const GOALS = [
...
const TEMPLATES = [
  ...
  },
  {
    ...
    structure: [
      ...
      { label: "Pause (Beat)", time: "12–14s", note: "Hold 1-2s before punchline — this is key" },
  ...
  },
  {
    ...
    structure: [
      { label: "Promise Hook", time: "0–4s", note: "Tell them exactly what they'll learn" },
  ...
  },
  {
    ...
    structure: [
      ...
      { label: "Community CTA", time: "50–60s", note: "Invite them into your world" },
    ],
...
export default function AIVideoEditor({ useAdmin }: { useAdmin?: boolean }) {
  ...
  const [inputType, setInputType] = useState<"script" | "url" | "describe" | "idea">("idea");
  ...
  const ideaMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest("POST", "/api/video/idea-builder", { concept, mode, goal, audience, style, platform, duration: targetDuration });
    },
    onSuccess: (data) => { setResult(data); setIsIdeaResult(true); setActiveTab("script"); setAppliedEdits(new Set()); },
  ...
  const exportPlan = () => {
    ...
    const lines = [
      ...
      `\n## Cut Suggestions`,
      ...(result.cuts || []).map((c: any) => `- ${c.timestamp} (${c.severity}): ${c.reason} → ${c.fix}`),
      `\n## Checklist`,
      ...(result.checklist || []).map((item: string) => `☐ ${item}`),
      `\n## Post Caption\n${result.captions?.postCaption || ""}`,
      `\n## Hashtags\n${(result.captions?.hashtags || []).join(" ")}`,
    ];
    if (result.fullScript) lines.unshift(`\n## Full Script\n${result.fullScript}`);
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">Powered by Google Gemini — describe an idea or paste your content</p>
            </div>
          </div>
          <Button variant="outline" size="sm" data-testid="btn-templates" onClick={() => setShowTemplates(v => !v)} className="border-primary/30 text-primary hover:bg-primary/10 gap-2">
            <Layers className="w-4 h-4" />Template Library{showTemplates ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
          </Button>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {TEMPLATES.map(t => (
                <div key={t.id} className="bg-card border border-card-border rounded-2xl overflow-hidden">
                  <button className="w-full flex items-center gap-3 p-4 text-left hover:bg-muted/10 transition-colors" onClick={() => setExpandedTemplate(expandedTemplate === t.id ? null : t.id)} data-testid={`template-${t.id}`}>
                  ...
                  {expandedTemplate === t.id && (
                    <div className="border-t border-card-border px-4 pb-4 space-y-3">
                      <div className="space-y-2 mt-3">
                        {t.structure.map((s, i) => (
                          <div key={i} className="flex items-start gap-3">
                            <span className="text-[10px] font-mono text-primary bg-primary/10 px-1.5 py-0.5 rounded w-14 text-center flex-shrink-0 mt-0.5">{s.time}</span>
                            <div><p className="text-xs font-semibold text-foreground">{s.label}</p><p className="text-[11px] text-muted-foreground">{s.note}</p></div>
                          </div>
                        ))}
                      </div>
                      <div className="bg-muted/10 rounded-xl p-3">
                        ...
                        {t.hooks.map((h, i) => <p key={i} className="text-xs text-foreground italic mt-1">› "{h}"</p>)}
                      </div>
                      <Button size="sm" variant="outline" className="w-full border-primary/30 text-primary hover:bg-primary/10 text-xs"
                        onClick={() => { setMode(t.mode); setInputType("idea"); setConcept(t.hooks[0]); setShowTemplates(false); toast({ title: `${t.name} template loaded` }); }}>
                        Use this template
                      </Button>
                    </div>
                  )}
        ...
        <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
          <div className="flex border-b border-card-border overflow-x-auto">
            {([
              { id: "idea", label: "Idea Builder", icon: Sparkles, desc: "Describe your video — AI writes everything" },
              { id: "script", label: "Paste Script", icon: FileText, desc: "Analyze your existing content" },
              { id: "url", label: "Video URL", icon: Link2, desc: "Instagram or YouTube link" },
          ...
          <div className="p-5 space-y-4">
            {inputType === "idea" && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-bold text-foreground">Describe your video idea</p>
                </div>
                <Textarea data-testid="input-concept" value={concept} onChange={e => setConcept(e.target.value)}
                  placeholder='e.g. "I want to make a 30-second reel about why most people fail at dieting, targeting busy professionals. Make it high energy like Alex Hormozi — fast cuts, bold captions, and a shocking stat to open."'
                  className="bg-background border-border text-sm min-h-[130px] resize-none" />
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">Platform</p>
                    <div className="flex gap-1.5">
                      {PLATFORMS.map(p => (
                        <button key={p.id} onClick={() => setPlatform(p.id)} className={`flex-1 py-1.5 px-2 rounded-lg text-[11px] font-semibold border transition-all ${platform === p.id ? "bg-primary/15 border-primary/40 text-primary" : "border-muted-foreground/20 text-muted-foreground hover:border-muted-foreground/40"}`}>
                          {p.label.split(" ")[0]}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">Target Duration</p>
            ...
            {inputType === "url" && (
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-2">YouTube URLs: Gemini analyzes the actual video. Instagram: extracts engagement data. Google Drive: paste the link + describe the content below.</p>
              </div>
            )}
            {inputType === "script" && (
              <Textarea data-testid="input-script" value={inputValue} onChange={e => setInputValue(e.target.value)} placeholder="Paste your video script, voiceover, or raw content idea here..." className="bg-background border-border text-sm min-h-[120px] resize-none" />
            )}
            {inputType === "describe" && (
              <Textarea data-testid="input-describe" value={inputValue} onChange={e => setInputValue(e.target.value)} placeholder='Describe your video or say how you want it to feel... e.g. "Make it Alex Hormozi style — fast cuts, bold captions, about why most people fail at sales"' className="bg-background border-border text-sm min-h-[100px] resize-none" />
            )}
        ...
        <Button data-testid="btn-analyze" onClick={handleGenerate} disabled={isPending} className="w-full h-12 text-sm font-black bg-primary text-black hover:bg-primary/90 gap-2">
          {isPending ? (
            ...
            <><Wand2 className="w-4 h-4" />{inputType === "idea" ? "Build My Video — Generate Full Plan" : "Analyze & Generate Edit Plan"}</>
          )}
        </Button>
        ...
        {isPending && (
          <div className="bg-card border border-card-border rounded-2xl p-6 space-y-3">
            {(inputType === "idea"
              ? ["Understanding your video concept...", "Writing your word-for-word script...", "Building timeline and shot list...", "Generating captions, hooks and visuals..."]
              : ["Analyzing your content with Google Gemini...", "Identifying cut points and weak spots...", "Building timestamped edit plan...", "Generating captions, hooks and checklist..."]
            ).map((step, i) => (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                {/* Script Tab (Idea Builder) */}
                {activeTab === "script" && result.fullScript && (
                  <div className="space-y-4">
                    <SectionHeader icon={FileText} title="Full Video Script" desc="Word-for-word — record exactly this. Markers show editing cues." color="from-green-500/20 to-green-500/5 border-green-500/30" />
                ...
                {/* Timeline / Edit Plan Tab */}
                {activeTab === "timeline" && (
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-3">
                      <SectionHeader icon={Play} title="Edit Timeline" desc="Click checkboxes to mark edits as done" color="from-blue-500/20 to-blue-500/5 border-blue-500/30" />
                      {appliedEdits.size > 0 && (
                        <Badge className="bg-green-500/20 text-green-400 border-green-500/30 border text-xs flex-shrink-0 mt-1">
                          {appliedEdits.size}/{result.timeline?.length} done
                        </Badge>
                      )}
                    </div>
                    ...
                    {result.timeline?.length > 0 && (
                      <div className="flex gap-0.5 h-5 rounded-lg overflow-hidden mb-2">
                        {result.timeline.map((seg: any, i: number) => (
                          <div key={i} className={`flex-1 ${appliedEdits.has(seg.id ?? i) ? "opacity-30" : "opacity-80"} ${TYPE_COLORS[seg.type] || "bg-muted"} hover:opacity-100 transition-opacity cursor-default`} title={`${seg.label} — ${seg.action}`} />
                        ))}
                      </div>
                    )}
                ...
                {activeTab === "hooks" && (
                  <div className="space-y-5">
                    <SectionHeader icon={Zap} title="Hook Optimizer" desc="Upgraded first-3-second hooks + audio recommendations" color="from-yellow-500/20 to-yellow-500/5 border-yellow-500/30" />
                    {result.hook && (
                      <>
                ...
                {activeTab === "checklist" && (
                  <div className="space-y-4">
                    <SectionHeader icon={ListChecks} title="Editing Checklist" desc="Step-by-step guide — tick as you go" color="from-green-500/20 to-green-500/5 border-green-500/30" />
                    <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" className="flex-1 border-primary/30 text-primary hover:bg-primary/10 text-xs gap-2" onClick={exportPlan}>
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => { setResult(null); setActiveTab("timeline"); setAppliedEdits(new Set()); }} className="flex-1 border-muted-foreground/20 text-muted-foreground text-xs gap-2" data-testid="btn-reset">
