  ...
  useEffect(() => {
    ...
    if (lastClipIdRef.current !== clip.id) {
      ...
      if (isFirstLoad || !nxtVid) {
        // First clip: load directly into current buffer — no buffer switch needed
        curVid.src = clip.blobUrl;
        applyProps(curVid);
        let done = false;
        const onReady = () => { if (done) return; done = true; curVid.currentTime = localTime; if (isPlayingRef.current) curVid.play().catch(() => {}); };
        curVid.addEventListener("loadedmetadata", onReady, { once: true });
        curVid.addEventListener("canplay",        onReady, { once: true });
        curVid.load();
      } else {
        // Subsequent clips: load into next buffer, then crossfade
        nxtVid.pause();
      ...
      <div className="h-screen bg-zinc-950 text-white flex flex-col overflow-hidden select-none">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800 bg-zinc-900 flex-shrink-0 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {exportUrl ? (
              ...
              <Button size="sm" className="bg-primary hover:bg-primary/90 text-black font-black gap-1.5 h-7 text-xs"
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-black min-w-0 gap-3 py-3">
            ...
            {/* Playback controls */}
            <div className="flex items-center gap-2">
              <button onClick={() => { setGlobalTime(0); setIsPlaying(false); }} className="text-zinc-500 hover:text-white" title="J">
                <SkipBack className="w-4 h-4" />
              </button>
              <button onClick={() => setGlobalTime(t => Math.max(0, t - 1/30))} className="text-zinc-500 hover:text-white" title="←">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button onClick={togglePlay}
                className="w-9 h-9 rounded-full bg-primary flex items-center justify-center hover:bg-primary/90 transition-colors" title="Space / K">
                {isPlaying ? <Pause className="w-4 h-4 text-black fill-black" /> : <Play className="w-4 h-4 text-black fill-black ml-0.5" />}
              </button>
              <button onClick={() => setGlobalTime(t => Math.min(totDur, t + 1/30))} className="text-zinc-500 hover:text-white" title="→">
                <ChevronRight className="w-4 h-4" />
              </button>
              <button onClick={() => { setGlobalTime(totDur); setIsPlaying(false); }} className="text-zinc-500 hover:text-white" title="L">
          ...
          <div className="w-64 border-l border-zinc-800 bg-zinc-900 flex flex-col flex-shrink-0 overflow-hidden">
            ...
            <div className="flex-1 p-3 space-y-3 overflow-y-auto text-xs">
              ...
              {activeTool === "none" && selectedClip && (
                <div className="space-y-3">
                  {/* Thumbnail */}
                  <div className="bg-zinc-800 rounded-xl overflow-hidden aspect-video relative">
                    {selectedClip.thumbnails[0]
                      ? <img src={selectedClip.thumbnails[0]} className="w-full h-full object-cover" alt="" />
                      : <div className="w-full h-full flex items-center justify-center"><Loader2 className="w-5 h-5 text-zinc-600 animate-spin" /></div>}
                    <div className="absolute bottom-1 left-1.5 text-[8px] font-mono text-white/70 bg-black/50 px-1 py-0.5 rounded">
                      {fmtTime(selectedClip.trimEnd - selectedClip.trimStart)}
                    </div>
                  </div>
                  {/* Filename */}
                  <p className="text-[10px] text-zinc-400 truncate font-medium" title={selectedClip.file.name}>
                  ...
                  <div className="space-y-1 text-[10px]">
                    ...
                    <div className="flex justify-between"><span className="text-zinc-500">Ken Burns</span><span className={selectedClip.kenBurns.enabled ? "text-blue-400" : ""}>{selectedClip.kenBurns.enabled ? "On" : "Off"}</span></div>
                    <div className="flex justify-between"><span className="text-zinc-500">Noise reduce</span><span className={selectedClip.noiseReduce ? "text-green-400" : ""}>{selectedClip.noiseReduce ? "On" : "Off"}</span></div>
                  </div>
                  {/* Actions */}
                  <div className="grid grid-cols-2 gap-1.5">
              ...
              {activeTool === "none" && !selectedClip && (
                <div className="space-y-3">
                  <div className="bg-zinc-800/60 rounded-xl p-3 space-y-1.5 text-[10px]">
                    <div className="flex justify-between"><span className="text-zinc-500">Clips</span><span className="font-mono">{clips.length}</span></div>
                    <div className="flex justify-between"><span className="text-zinc-500">Total duration</span><span className="font-mono text-primary">{fmtTime(totDur)}</span></div>
        ...
        <div className="border-t border-zinc-800 bg-zinc-950 flex-shrink-0" style={{ height: "200px" }}>
          <div className="h-full overflow-x-auto overflow-y-hidden relative" ref={timelineRef}
            onMouseDown={onTimelineMouseDown} style={{ cursor: "default" }}>
